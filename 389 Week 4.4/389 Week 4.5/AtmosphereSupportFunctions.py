...
def BunchFunction(NumberofParticles):
    ...
    for i in range(1,NumberofParticles+1):
        ...
        Proton = CosmicRay([0,8E4,0], [0.4*constants.speed_of_light*np.cos(theta), -(0.43 + (0.566*i)/NumberofParticles)*constants.speed_of_light, 0.4*constants.speed_of_light*np.sin(theta)], [0,0,0],'Proton %s'%(i+1), 1.67E-27, 1, 0) # 80E4 is top of mesosphere, cosmic ray speed range 0.43c - 0.996c
        PList.append(Proton)
    return(PList)
